...
function timeToMinutes(input: string): number | null {
  ...
  if (!hasPM && !hasAM && hours >= 1 && hours <= 6) hours += 12;
...
export function detectSlotSelection(
  ...
  if (/\b(first|1st|earliest|soonest)\b/i.test(userMessage)) return offeredSlots[0] ?? null;
  if (/\b(second|2nd)\b/i.test(userMessage))                  return offeredSlots[1] ?? null;
  if (/\b(third|3rd)\b/i.test(userMessage))                   return offeredSlots[2] ?? null;
...
export function extractPatientName(messages: any[]): string | null {
  ...
  // Explicit introduction anywhere in conversation
  ...
  // Bot asked for name — extract from next user message
  for (let i = 1; i < messages.length; i++) {
    ...
    // Strip phone number AND trailing filler phrases
    const stripped = curr.content
      .replace(/\+?\d[\d\s\-().]{8,}/g, "")          // remove phone number
      .replace(/\band\b.*(phone|number|contact|reach).*/i, "") // "and the phone number is..."
      .replace(/,?\s*my\s+(phone|number|contact|mobile)\b.*/i, "") // "my phone is..."
